...
注意了，ES6新增的 `class` 关键字所声明的对象，**仍然是基于原型的，`class` 事实上只是一个语法糖**。当然，`super`, `extends` 这些和 class 搭配的关键字同样也是一个语法糖。所以说，不需要对 JS 中的 class 参杂一些其他的理解，它仅仅是一个语法糖而已，而且严格来说，作为一个动态语言，不存在 "class" 这个概念。
